...
import { MoveLexer } from "./antlr/MoveLexer";
import { MoveParser } from "./antlr/MoveParser";
import {
  ASTNode,
  astNodeTypes,
  ASTNodeTypeString,
...
export function tokenize(input: string, options: TokenizeOptions = {}): any {
  ...
  const lexer = new MoveLexer(inputStream);
...
export function parse(input: string, options: ParseOptions = {}): ParseResult {
  ...
  const parser = new MoveParser(tokenStream);
